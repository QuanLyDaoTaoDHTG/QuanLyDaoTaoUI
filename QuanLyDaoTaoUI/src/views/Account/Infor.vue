<template>
  <label>Họ tên</label>
  <el-input v-model="thongtinsv.hoTen" size="large" placeholder="Họ tên" />
  <div class="block">
  ...
  <label>Email</label>
  <el-input v-model="thongtinsv.email" size="large" placeholder="Email" />
  ...
  <el-input v-model="thongtinsv.noiSinh" size="large" placeholder="Nơi sinh" />
...
const getData = async () => {
  await axiosInstance
    .get("Account/infor/" + useRoute().params.Id.toString())
    .then((response) => {
...
async function handleSave() {
  ...
  console.log(thongtinsv.value);
  await axiosInstance
    .put("Account/changeInfor", thongtinsv.value)
    .then((response) => {
      if (response.data.isSuccess && response.data.data != null) {
